...
class PaymentViewSet(viewsets.GenericViewSet):
    ...
    def _has_admin_access(self, request):
        return self._has_internal_admin_access(request) or "admin" in self._request_roles(request)
    ...
    def confirm(self, request, pk=None):
        ...
        if not payment:
            return Response({"detail": "Payment not found."}, status=status.HTTP_404_NOT_FOUND)
        if not payment.can_transition_to(Payment.Status.PAID):
            return Response(
        ...
        if error_response:
            return error_response
        was_already_paid = payment.status == Payment.Status.PAID
        self._transition(payment, Payment.Status.PAID)
