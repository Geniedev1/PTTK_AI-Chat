...
class CartSessionFlowTest(TestCase):
    ...
    @patch("cart.views.requests.get")
    def test_add_product_rejects_quantity_above_stock(self, mock_get):
        mock_get.return_value = self._product_response()
        Cart.objects.create(session_key="session-1", product_id=1, quantity=9, price_snapshot="99.99")
        ...
        response = self.add_view(request)

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["available_stock"], 10)
        cart_item = Cart.objects.get(session_key="session-1", product_id=1)
        self.assertEqual(cart_item.quantity, 9)
    ...
    @patch("cart.views.requests.get")
    def test_update_quantity_rejects_quantity_above_stock(self, mock_get):
        mock_get.return_value = self._product_response()
        Cart.objects.create(session_key="session-2", product_id=1, quantity=3, price_snapshot="99.99")
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.data["available_stock"], 10)
        cart_item = Cart.objects.get(session_key="session-2", product_id=1)
        self.assertEqual(cart_item.quantity, 3)
